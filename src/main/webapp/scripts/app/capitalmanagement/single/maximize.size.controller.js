...
angular.module('dEACMApp')
    .controller('SingleDMUMaximizeSizeController', function ($scope, $modal, $state, $stateParams, Principal, CapitalManagement) {
        ...
        if( $stateParams.ke ) $scope.ke = $stateParams.ke;
        ...
        $scope.execute = function(){
        	...
        	if( !$scope.validateRequiredNumber($scope.ke, "l2 efficiency coefficient") ) return;
    		...
    		CapitalManagement.singleDMUMaxSize(
    			{"target": dmu0, "capital": parseFloat($scope.capital), "ke": parseFloat($scope.ke), "dmus": $scope.dmus},
    			function(result){
    				$state.go('home.single-maximize-size.result', { "solution": result, "kaoDistance": $stateParams.kaoDistance });
    			},
    			function(error){
