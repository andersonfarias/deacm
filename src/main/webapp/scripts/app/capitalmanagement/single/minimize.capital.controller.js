...
angular.module('dEACMApp')
    .controller('SingleDMUMinimizeCapitalController', function ($scope, $modal, $state, $stateParams, Principal, CapitalManagement) {
        ...
        $scope.execute = function(){
    		...
    		CapitalManagement.singleDMUMinCapital(
    			{"target": dmu0, "ks": parseFloat($scope.ks), "ke": parseFloat($scope.ke), "dmus": $scope.dmus},
    			function( result ){
    				$state.go('home.single-minimize-capital.result', { "solution": result, "kaoDistance": $stateParams.kaoDistance });
    			},
    			function(error){
