...
public class MinimizeCapital implements LinearProgrammingModel {
	...
	public static final String DMU_SET_MINIMIZE_MODEL_ALTRUISTIC_FILE = "set_dmu-minimize_capital_altruistic.mod";

	public static final String DMU_SET_MINIMIZE_MODEL_ALTRUISTIC_FILE_SUPER_EFICIENCY = "set_dmu-minimize_capital_altruistic_se.mod";

	public static final String DMU_SET_MINIMIZE_MODEL_SELFISH_FILE = "set_dmu-minimize_capital_selfish.mod";

	public static final String DMU_SET_MINIMIZE_MODEL_SELFISH_FILE_SUPER_EFICIENCY = "set_dmu-minimize_capital_selfish_se.mod";
	...
	public Solution solve() {
		...
		try {
			...
			Parameter tdParam = ampl.getParameter( "td" );
			tdParam.set( td );
			...
			DataFrame capitalDistribution = c.getValues();
			for ( int i = 0; i < capitalDistribution.getNumRows(); i++ ) {
				String targetDMUName = (String) capitalDistribution.getRowByIndex( i )[0];
				BigDecimal targetDMUCapital = new BigDecimal( (Double) capitalDistribution.getRowByIndex( i )[1], LinearProgrammingModel.DEFAULT_MATH_CONTEXT );

				for ( DMU target : targets ) {
					if ( target.getName().equals( targetDMUName ) ) {
						target.setCapital( targetDMUCapital.doubleValue() );
						for ( DMU dmu : dmus ) {
							if ( dmu.getName().equals( target.getName() ) ) {
			...
			BigDecimal tdB = new BigDecimal( td, LinearProgrammingModel.DEFAULT_MATH_CONTEXT );
			BigDecimal zB = new BigDecimal( z.value(), LinearProgrammingModel.DEFAULT_MATH_CONTEXT );
			...
			return new SetDMUCapitalMinimizationSolution( tdB.doubleValue(), zB.doubleValue(), kaoDistanceB.doubleValue(), dmus, targets, inputsOffsets, outputsOffsets, executeWithSuperEfficiency, mode );
		} catch ( Exception e ) {
			e.printStackTrace();
	...
	public MinimizeCapital( String modelFilePath, List<DMU> targets, List<DMU> dmus, Double td, Double kaoDistance, boolean executeWithSuperEfficiency, ModelMode mode ) {
		super();
