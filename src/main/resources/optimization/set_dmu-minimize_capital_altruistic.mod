...
subject to distanceminimization: (sum {k in O} (E[k] - (sum {r in output} ((Y[k,r]+f[k,r])*u[r]) /  sum {i in input} (A[k,i]*(X[k,i]+d[k,i])*v[i])))^2 )^1/2 = td*l;


subject to sizechange1 {k in S}: (sum {r in output} (Y[k,r]+f[k,r])*u[r])*(sum {i in input} A[k,i]*(X[k,i]+d[k,i])*v[i]) >= 2*t[k]*s[k];

subject to sizechange2 {k in S}: (sum {r in output} (Y[k,r]+f[k,r])*u[r])*(sum {i in input} A[k,i]*(X[k,i]+d[k,i])*v[i]) <= 2;

subject to capitalconsumption {k in S}: sum {i in input} A[k,i]*d[k,i] = c[k];

subject to efficiencymaintenance {k in S}: sum {r in output} ((Y[k,r]+f[k,r])*u[r]) - sum {i in input} (A[k,i]*(X[k,i]+d[k,i])*v[i]) <= 0;
